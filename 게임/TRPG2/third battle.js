...
logMessage("마법사 와 함께", 'green');
logMessage("악취나는 동굴", 'green');
logMessage("에서 발견한");
logMessage("마법진을 작동 시키려고 합니다", 'green');

logMessage("마법진의 반대 위치는 어딘지 모를 곳이지만");
logMessage("떨고있는 마법사를 끌고 발동후 포탈에 들어갑니다");

logMessage("포탈에 들어서자 끔찍한 실험이 이뤄진듯한");
logMessage("실험실에 도착", 'green');
...
function Battle(){
    ...
    logMessage("조잡하게 만들어진"+monster.name+"를 조우" , 'red');
    logMessage(hero.name+"을(를) 보고는 괴성을 지르며 "+monster.name+"가 달려듭니다", 'red');
    logMessage(hero.name + "과" + monster.name + "이 전투", 'red');
    ...
    while(true){
        ...
        if(monster.hp <= 0){
            ...
            if(cnt == 4){
                logMessage("여러 생물이 뒤엉켜 고통에 몸부림 치는 키메라를 안식에 들게했습니다", 'green');
                logMessage(hero.name + " 은 잠시 눈을 감았다 뜬 뒤 앞으로 나아갑니다", 'green');
                logMessage("강철로 만들어진듯한 문이 저 앞에 보입니다", 'green');
                logMessage(hero.name + " 은 망설이지 않고 앞으로 나아갑니다", 'green');
                Middle_boss_battle();
                break;
            }
...
function Boss_battle() {
    ...
    logMessage("동공 안에는 커다란 기둥으로 보이는 돌 기둥이 세워져있습니다", 'green');
    logMessage("가까이 다가가자 코를 찌르는 악취가 느껴지며", 'green');
    logMessage("동그란 무언가를 끼울 수 있는 홈이 있습니다", 'green');
    logMessage(hero.name+" 은 자연스럽게  green_correction 를 끼워 봅니다", 'green');

    logMessage("돌 기둥이 열리고", 'red');
    logMessage("세월의 흔적이 가득하지만 느껴지는 기세는 보이는 세월 만큼 거대하게 느껴집니다", 'red');
    logMessage("마법사중의 마법사 "+boss.name + "과 조우! 전투를 시작합니다", 'red');
    ...
    while(true){
        ...
        if( boss.hp <= 0){
            ...
            hero.it.push(boss_item);
                ...
                logMessage(boss.name + " 을 쓰러트리고 주위를 둘러봅니다", 'green');
                logMessage(" 이번에도 미약하게 빛이 나는", 'green');
                logMessage("마법진을 발견합니다", 'green');
                logMessage("마법사는 안전한 포탈이라고 말해줍니다", 'green');
                logMessage("포탈을 사용합니다", 'green');
