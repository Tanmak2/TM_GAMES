...
function lotto_change() {
  ...
  lotto_img.src = '../../image/' + lotto_ran + '.png';
    ...
    if (isNum == false) {
      ...
      if (lotto_count < 6) {
        ...
        for (var i = 0; i < lotto_nums.length; i++) {
          ...
          lotto_result_num.src = '../../image/' + lotto_nums[i] + '.png';
        }
      } else {
        var lotto_result_num = document.getElementById('lotto_num7');
        lotto_result_num.src = '../../image/' + lotto_nums[6] + '.png';
      }
      lotto_count++;
      lotto_ran_count = 0;
...
function lotto_num_chk(chk_number) {
  ...
  if (chk_number.checked == true) {
    ...
    for (var i = 0; i < user_lotto.length; i++) {
      ...
      lotto_select_num.src = '../../image/' + user_lotto[i] + '.png';
    }
    chk_count++;
  }
    ...
    for (var i = 0; i < user_lotto.length; i++) {
      if (user_lotto[i] == user_chk.substr(8)) {
        for (var j = 0; j < user_lotto.length; j++) {
          var lotto_select_num = document.getElementById(
            'user_select_' + (j + 1),
          );
